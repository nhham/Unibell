            ...
            ScrollView(.vertical) {
                VStack {
                    ...
                    VStack {
                        ScrollView(.horizontal) {
                            HStack {
                                TextField("Enter new embed link", text: $embedModel.newLink)
                                    .textFieldStyle(RoundedBorderTextFieldStyle())
                                    .padding(.horizontal)
                                    .frame(width: bound.size.width, height: 100)
                                    .scaledToFit()
                                
                                
                                Button(action: {
                                    embedModel.updateEmbedLink(userUID: loginModel.userUID)
                                }) {
                        ...
                        Text(embedModel.updateStatus)
                            .foregroundColor(!embedModel.updateStatus.contains("Successfully") ? .red : .green)
                            .font(.system(size: xSmallFontSize))
